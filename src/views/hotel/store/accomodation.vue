...
export default {
  ...
  methods: {
    ...
    addNew(item) {
      ...
      const it = this.hotel.accomodation.findIndex((x) => x._id === id);
      ...
      const is = this.hotel.accomodation.findIndex((x) => x._id === item._id);
      this.hotel.accomodation = this.changePosition(this.hotel.accomodation, it, is + 1);
    },
    deleteAccomadationType(item, itemId, type) {
      if (type === 'age') {
        const it = this.hotel.accomodation.findIndex((x) => x._id === itemId);
        // const is = hotel.accomodation[it].age.findIndex((x) => x._id === item._id);
        this.hotel.accomodation[it].age = this.hotel.accomodation[it].age.filter((x) => x._id !== item._id);
        if (this.hotel.accomodation[it].age.length < 1) {
          this.hotel.accomodation = this.hotel.accomodation.filter((x) => x._id !== item.accomodation_id);
        }
      } else {
        this.hotel.accomodation = this.hotel.accomodation.filter((x) => x._id !== itemId);
      }
    },
    changeAge(age, item) {
      if (age.id === 1) {
        const i = this.hotel.accomodation.findIndex((x) => x._id === item._id);
            ...
            <ol>
              <li style="list-style: none;">
                <div class="row text-center">
                  ...
                  <div class="col-1">Mount/Year</div>
            ...
            <ol class="px-3" style="list-style-type: upper-alpha">
              <li class="my-2" v-for="item in hotel.accomodation.filter((a) => a.room_id === room._id)" :key="item">
                <div class="row">
                  <div class="col-3">
                    <label>
                      <span>{{item.adultSize}} Adult </span>
                      <span v-if="item.childSize != 0">+ {{item.childSize}} Child </span>
                    </label>
                    <label v-if="item.extBed.active"> &nbsp;(Extra Bed. <input type="checkbox" v-model="item.extBed.value" class="form-check-input"/>)</label>
                  ...
                  <div v-if="item?.age.length > 0" class="col-9">
                    <div class="row" v-for="age in item.age" :key="age">
                      ...
                      <div class="col-2">
                        <input v-model="age.min" @focus="$event.target.select()" type="number" step="0.01" class="form-control form-control-sm" :placeholder="age._id + '. child age start'">
                      </div>
                      <div class="col-2">
                        <input v-model="age.max" @focus="$event.target.select()" type="number" step="0.01" class="form-control form-control-sm" :placeholder="age._id + '. child age end'">
                      </div>
                      ...
                      <div class="col-2 d-flex justify-content-center">
                        <button class="btn" @click="deleteAccomadationType(age, item._id, 'age')" type="button">
                  ...
                  <div v-else class="col-9">
                    <div class="row">
                      ...
                      <div class="col-2 offset-7 d-flex justify-content-center">
                        <button class="btn" @click="deleteAccomadationType(item, item._id, 'adult')" type="button">
                          <span class="ri ri-delete-bin-2-line"></span>
                        </button>
                      </div>
                    </div>
